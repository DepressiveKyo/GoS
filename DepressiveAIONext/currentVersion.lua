Data = {
    Core = {
        Version = 0.01,
        Changelog = "Initial core release",
    },
    Loader = {
        Version = 0.01,
    },
    Champions = {
    Aatrox = { Version = 3.0, Changelog = "Aatrox" },
    Ahri = { Version = 0.23, Changelog = "Ahri" },
    Akali = { Version = 0.47, Changelog = "Akali" },
    Aphelios = { Version = 1.0, Changelog = "Aphelios" },
    Cassiopeia = { Version = 0.01, Changelog = "Cassiopeia" },
    Elise = { Version = 1.25, Changelog = "Elise" },
    Fiora = { Version = 2.2, Changelog = "Fiora" },
    Irelia = { Version = 1.1, Changelog = "Irelia" },
    Jhin = { Version = 1.0, Changelog = "Jhin" },
    Jinx = { Version = 2.2, Changelog = "Jinx" },
    Katarina = { Version = 1.1, Changelog = "Katarina" },
    LeeSin = { Version = 5.3, Changelog = "LeeSin" },
    Lillia = { Version = 1.0, Changelog = "Lillia" },
    Lulu = { Version = 1.0, Changelog = "Lulu" },
    Mel = { Version = 0.2, Changelog = "Mel" },
    Riven = { Version = 1.0, Changelog = "Riven" },
    TwistedFate = { Version = 1.9, Changelog = "TwistedFate" },
    Varus = { Version = 1.0, Changelog = "Varus" },
    Vayne = { Version = 4.0, Changelog = "Vayne" },
    Yasuo = { Version = 4.2, Changelog = "Yasuo" },
    Yone = { Version = 3.1, Changelog = "Yone" },
    Zed = { Version = 1.0, Changelog = "Zed" },
    }
}



