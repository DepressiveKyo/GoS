...
function Ahri:Mode()
    ...
    if _G.DepressiveOrbwalker then
        local mode = _G.DepressiveOrbwalker:GetMode()
        local M = _G.DepressiveOrbwalker.MODES
        if mode == M.COMBO then return "Combo" end
        if mode == M.HARASS then return "Harass" end
        if mode == M.LANECLEAR or mode == M.JUNGLECLEAR then return "Clear" end
    end
