...
-- Jungle items that affect smite damage (new season items)
local JungleItems = {
    -- New jungle items - IDs will be detected automatically
    ["Scorchclaw Pup"] = {charges = true, maxCharges = 40},
    ["Gustwalker Hatchling"] = {charges = true, maxCharges = 40},
    ["Mosstomper Seedling"] = {charges = true, maxCharges = 40}
}

-- Table to store detected item IDs
local DetectedItemIDs = {}
...
local function DetectJungleItemIDs()
    for slot = 0, 5 do
        local item = myHero:GetItemData(slot)
        if item and item.itemID and item.itemID > 0 then
            local gameItem = Game.Item(item.itemID)
            if gameItem and gameItem.name then
                local itemName = gameItem.name
                
                if JungleItems[itemName] then
                    if not DetectedItemIDs[itemName] then
                        DetectedItemIDs[itemName] = item.itemID
                    end
                end
            end
        end
...
function AutoSmite:LoadMenu()
    ...
    self.Menu.drawing:MenuElement({id = "jungleItems", name = "Show Smite Info", value = true})
end
...
function AutoSmite:Draw()
    ...
    if self.Menu.drawing.jungleItems:Value() then
