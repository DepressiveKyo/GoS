...
-- Throttling utility for iteration-heavy logic (does NOT affect Draw frequency)
function Elise:ShouldRun(key, interval)
    self._nextRun = self._nextRun or {}
    local t = GameTimer()
    local nextTime = self._nextRun[key] or 0
    if t >= nextTime then
        self._nextRun[key] = t + interval
        return true
    end
    return false
end
...
function Elise:Tick()
    ...
    -- Refresh enemy list at most every 0.5s
    if self:ShouldRun("EnemyRefresh", 0.5) then
        Enemys = GetEnemyHeroes()
    end

    -- Prediction tracking update throttled (every 0.15s)
    if PredictionLib and self:ShouldRun("PredRefresh", 0.15) then
        for i = 1, #Enemys do
            local enemy = Enemys[i]
            if enemy and enemy.valid then
                pcall(function()
                    PredictionLib.GetPredictedPosition(enemy, 0.1)
                end)
            end
        end
    end

    -- Update form & status each tick (cheap)
    self:UpdateFormState()

    -- Auto Rappel logic throttled slightly (every 0.05s)
    if self:ShouldRun("AutoRappel", 0.05) then
        self:AutoRappel()
    end

    -- Mode logic throttled (every 0.05s) to reduce per-frame heavy loops
    if self:ShouldRun("ModeLogic", 0.05) then
